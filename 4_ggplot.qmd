  ...
  - name: Jessica Cooperstone
    ...
    affiliation: 
      - name: Horticulture and Crop Science, Ohio State University
        url: https://cfaes.osu.edu/person/jessica-cooperstone 
date: 2026-06-17
...
::: exercise
#### {{< fa user-edit >}} **Challenge 1: Modifying a plot**
...
Try mapping `x = year` and `y = lifeExp`.

#### {{< fa circle-info >}} Explanation

Adjust the existing plot by mapping `x = year` and `y = lifeExp`, and use `geom_point()` since we want a scatterplot. We pipe the dataframe `gapminder` to provide the underlying data for creating the plot.
...
::: exercise
#### {{< fa user-edit >}} **Challenge 2: Coloring by a variable**
...
Modify your plot to color your points by continent.

```{webr}
#| autorun: false
#| caption: ""
# paste your code from Challenge 1
...
#### {{< fa lock >}} Need another hint?

Try setting `color = continent` within your aesthetic mappings.

#### {{< fa circle-info >}} Explanation

Adjust the existing plot by adding `color = continent` to color observation based on its value in the column `continent`.
...
::: exercise
#### {{< fa user-edit >}} **Challenge 3: Adjusting code order**
...
Code is run in the order it is provided, with each subsequent layer placed on **top** of the one that came before. So, when you write your code with points first, then lines lower down, the lines will be on top of the points.

We can see that in this situation since `color` is mapped only to the `geom_point()` layer.
...
gapminder |>
  ...
  scale_x_log10() +
  geom_smooth(
    method = "lm", # smooth with a linear model ie "lm"
    linewidth = 3, # increase thickness of the line
    se = FALSE
  ) # turn off plotting of a confident internal
```
...
::: exercise
#### {{< fa user-edit >}} **Challenge 4A: Modifying color and size**
...
Don't put `color` and `size` inside your `aes()`.

#### {{< fa lock >}} Need another hint?

The equivalent for `linewidth` for points is called `size`.

#### {{< fa circle-info >}} Explanation

Adjust the existing plot by setting `color` and `size` inside `geom_point()` - both are outside of an `aes()` statement because in this case, we are not mapping a variable to an aesthetic, we are simply setting a parameter of the plot.
...
::: exercise
#### {{< fa user-edit >}} **Challenge 4B: Changing shape and adding coloring**
...
You can learn more about shape codes [here](https://ggplot2.tidyverse.org/reference/scale_shape.html#details).
...
gapminder |>
  ...
  geom_point(
    shape = 17, # 17 is a closed triangle
    alpha = 0.5
  ) +
  scale_x_log10() +
...
gapminder |>
  ...
  geom_point(
    ...
    alpha = 0.5,
    color = "black"
  ) + # color controls outside, fill controls inside
  scale_x_log10() +
  geom_smooth(method = "lm", linewidth = 1) # smooth with a linear model ie "lm"
```
...
gapminder_americas |>
  ...
  theme_bw() + # change to a black and white theme
  theme(
    text = element_text(family = "AppleGothic"), # change all fonts
    ...
    axis.text.x = element_text(
      angle = 45, # years on the x on a 45 deg angle
      vjust = 0.7
    )
...
```{r, eval=FALSE}
ggsave(
  filename = "results/lifeExp.png", # file path and name
...
::: exercise
#### {{< fa user-edit >}} **Challenge 5: New plots with adjusted theming**
...
Set labels within `labs().` Adjust theming with `theme()`, and you can learn more about all the variables that can be set in `theme()` [here](https://ggplot2.tidyverse.org/reference/index.html#themes). Check out the [complete themes](https://ggplot2.tidyverse.org/reference/ggtheme.html).
...
gapminder |>
  ...
  labs(
    ...
    title = "Life expectancy across continents from 1952-2007"
  ) # add title
```
