...
the kinds of task that are commonly referred to as "data wrangling".
...
some more advanced than the ones we'll cover today,
but also some that are conceptually very simple
(e.g. `rename()` to change column names and `select()` to select columns)
that we encourage you to [explore](#bonus-material-for-self-study) on
[your own](https://dplyr.tidyverse.org/articles/dplyr.html). 
...
As you could see above, the `gapminder` data frame is referred to as a
*"tibble"*, which is just a slight variation on data frames from the tidyverse^[
...
Also, two of the columns are referred to as `<fct>`, which is short for `factor`,
[an alternative to](./2_vectors.qmd#factors) `character` commonly used
for categorical data.

Here is an overview of all columns in this data frame:
...
Other dplyr functions work equivalently in this regard.
